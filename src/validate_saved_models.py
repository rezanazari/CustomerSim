...
if __name__ == '__main__':
    ...
    # roc(to_categorical(y_test), y_score, name="../results/kdd98_propagation_classifier_roc3.pdf")

    model = KDDClassifier3()
    model.build(input_shape=[100, 10])
    model.load_weights(file_name)
    y_score = model.predict(x_test)
    # y_score = y_prob.argmax(axis=-1)
    roc(to_categorical(y_test), y_score, name="../results/kdd98_propagation_classifier_roc4.pdf")