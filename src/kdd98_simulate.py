...
def propagate(data, classifier, regressor, policy, threshold=0.275, periods=12, orig_actions=None):
    ...
    for t in range(periods):
        ...
        donations_occurred = 1 * (
                    np.random.binomial(8, threshold, donation_prob.shape[0]) / 8.0 < np.apply_along_axis(binom, 0,
                                                                                                         donation_prob))
        ...
        # Avg. Past Donation
        customers[t + 1, :, 2] = (customers[t, :, 2] * customers[t, :, 1] + donations[t]) / (
                    customers[t + 1, :, 1] + 1 * (customers[t + 1, :, 1] == 0))
...
plot_validate(orig_D.sum(0), D.sum(0), xlab="Total Donations", ylab="Probability Mass",
              name="../results/kdd98_total_donations.pdf",
...
plot_validate(orig_S[18, :, 0], S[18, :, 0], xlab="End-Period Recency", ylab="Probability Mass",
              name="../results/kdd98_end_recency.pdf",
