...
purchase_data = ps.pivot_table(trns, values=['purchasequantity', 'purchaseamount'], index=['id', 'period', 'cacobr'],
                               aggfunc=np.sum)
offer_data = ps.pivot_table(full_offer_history, values=['offer_ind', 'quantity', 'offervalue'],
                            index=['id', 'period', 'cacobr'], aggfunc=np.sum)
...
for p in range(1, len(trns_period)):
    ...
    data[:, p, :, 2] = (data[:, p - 1, :, 1] * data[:, p - 1, :, 2] + data[:, p - 1, :, 8] * data[:, p - 1, :, 9]) / (
                data[:, p, :, 1] + 1 * (data[:, p, :, 1] == 0))
